...
test("dashboard delivery serves files and falls back only for app navigation", async () => {
  ...
  try {
    ...
    const asset = await handle(new Request(assetUrl), assetUrl);
    ...
    const route = await handle(
      new Request(routeUrl, { headers: { accept: "text/html,application/xhtml+xml" } }),
      routeUrl,
    );
